...
def test_page_object():
    ...
    product = productCategoryPage(driver)
    product.openProducts(3)

    page_achat = ProductPage(driver)
    page_achat.Buy()
    page_achat.Choose_Delivery_Method()
    page_achat.Enter_Zip_Code()
    page_achat.Select_First_Store()

    Message_attendu = "1 produit indisponible dans ce magasin."
    assert page_achat.Get_Availability_Status() == Message_attendu
